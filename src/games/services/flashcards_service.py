...
class FlashcardsService:
    ...
    def start_session(self, user_id: str, body):
        wl = wordlists_service.get_word_list(user_id, body.wordListId)
        if wl is None:
            raise APIError("unknown_list", "Word list not found", 404)

        if body.selectedWordIds:
            words = self.dao.fetch_subset_words(body.wordListId, body.selectedWordIds)
        else:
            words = self.dao.fetch_list_words(body.wordListId)

        if not words:
            raise APIError("validation_error", "No words available", 400)

        session_id = f"fs_{uuid4().hex}"
        self.dao.create_session(session_id, user_id, body.wordListId, len(words))
    ...
    def record_result(self, user_id: str, session_id: str, body):
        ...
        wl = wordlists_service.get_word_list(user_id, wl_id)
        if not wl:
            raise APIError("unknown_list", "List not found", 404)

        wl_words = self.dao.fetch_list_words(wl_id)
        wl_ids = {w["id"] for w in wl_words}
        if body.wordId not in wl_ids:
            raise APIError("not_in_list", "Word not in list", 400)

        self.dao.insert_result(session_id, body.wordId, body.isCorrect, body.attempts, body.timeSpent)
        self.dao.update_word_stats(body.wordId, body.isCorrect)
        self.dao.update_session_progress(session_id, body.isCorrect)

        return {"ok": True}

    def complete_session(self, user_id: str, session_id: str, body):
        session = self.dao.get_session(session_id)
        if not session:
            raise APIError("not_found", "Session not found", 404)
        if session["user_id"] != user_id:
        ...
        self.dao.complete_session(session_id, body.progress.dict() if body.progress else None)
